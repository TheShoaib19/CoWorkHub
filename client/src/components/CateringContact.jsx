...
export default function CateringContact({Catering}) {
        ...
        try {
            const res = await fetch('/api/contact/sendEmail', {
                ...
                body: JSON.stringify({
                    ...
                    to: landlord.email,
                    subject: `${Catering.name}`, //Sending the listing name in the subject variable for contact.controller.jsx
                    text: message
                })
            });
            if (!res.ok) {
    ...
    useEffect(()=>{
        const fetchLandlord = async () => {
            try {
                const res = await fetch(`/api/user/${Catering.userRef}`);
                const data = await res.json();
                setLandlord(data);
            } catch (error) {
                console.log(error);
    ...
    <ToastContainer />
        {landlord && (
            <div className='flex flex-col gap-2'>
                <p>Contact <span className='font-semibold'>{landlord.username}</span> for <span className='font-semibold'>{Catering.name.toLowerCase()}</span></p>
