...
export default function ReviewFormCategory({ user, Catering }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post('/api/reviews/catering/create', { user: user._id, catering: Catering._id, stars, comment });
            toast.success('Review submitted successfully', { autoClose: 2000 }); // Display toast for 5 seconds
            setShowForm(false);
            setTimeout(() => {
                window.location.reload();
            }, 2100);
        } catch (error) {
            toast.error(error.message, { autoClose: 2000 }); // Display toast for 5 seconds
        }
    };
    ...
    return (
        <div>
            ...
            <div className={`transition-all duration-500 overflow-hidden ${showForm ? 'max-h-screen' : 'max-h-2'}`}> {/* Ye div hatadena and sirf jumping wapas ayegi */}
                {showForm && (
